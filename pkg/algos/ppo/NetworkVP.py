...
class NetworkVP(object):
    ...
    def _create_graph(self):
        ...
        self.ad = tf.placeholder(tf.float32, [None], name='Advantage')
        self.action_old_prob = tf.placeholder(tf.float32, [None], name='prob')
        ...
        self.fc2 = self.dense_layer(self.fc1, 8, 'fc2')
        self.d1 = self.dense_layer(self.fc2, 8, 'dense1')
        ...
        r_theta = self.selected_action_prob/tf.maximum(self.action_old_prob, Config.MIN_OLD_ACTION_PROB)
    
        self.cost_p_1 = tf.minimum(r_theta*self.ad, tf.clip_by_value(r_theta, 1-Config.EPSILON, 1+Config.EPSILON)*self.ad)
    ...
    def compute_gradients(self, x, y_r, a, p_, ad_):
        feed_dict = self.__get_base_feed_dict()
        feed_dict.update({self.x: x, self.y_r: y_r, self.action_index: a, self.action_old_prob: p_, self.ad: ad_})
