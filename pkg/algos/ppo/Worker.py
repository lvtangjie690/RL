...
class Worker(Process):
    ...
    def _accumulate_rewards(experiences, discount_factor, terminal_reward, done):
        ...
        for t in reversed(range(0, exp_length)):
            ...
            experiences[t].reward_sum = reward_sum
    ...
    def convert_data(self, experiences):
        ...
        r_ = np.array([exp.reward_sum for exp in experiences])
        p_ = np.array([exp.action_prob for exp in experiences])
        ad_ = np.array([exp.advantage for exp in experiences])
        return x_, r_, a_, p_, ad_
    ...
    def select_action(self, prediction, is_test=False):
        ...
        return action, prediction[action]
    ...
    def run_episode(self, test=False):
        ...
        while not done:
            ...
            action, action_prob = self.select_action(prediction, is_test=test)
            state, reward, done, next_state = self.game.step(action)
            reward /= PkgConfig.REWARD_SCALE
            # state, action, reward, done, next_state
            reward_sum += reward
            exp = Experience(state, action, prediction, reward, done, action_prob, value)
            ...
            if done or time_count == PkgConfig.TIME_MAX:
                ...
                x_, r_, a_, p_, ad_ = self.convert_data(updated_exps)
                yield x_, r_, a_, p_, ad_, reward_sum
    ...
    def check_and_update(self):
        if len(self.exp_cache) < 10:
            return
        for _ in range(Config.EPOCH_COUNT):
            for x_, r_, a_, p_, ad_ in self.exp_cache:
                # compute gradients and send to the master
                gradients = self.model.compute_gradients(x_, r_, a_, p_, ad_)
    ...
    def run(self):
        ...
        while True:
            total_reward = 0
            total_length = 0
            for x_, r_, a_, p_, ad_, reward_sum in self.run_episode():
                total_reward += reward_sum
                total_length += len(r_)
                self.exp_cache.append((x_, r_, a_, p_, ad_))
